  ...
  btns.forEach(function (btn) {
    btn.addEventListener("click", function () {
      if (this.classList.contains("active-style")) {
        changeClass(this, "toggle");
      } else {
        btns.forEach(function (btn) {
          changeClass(btn, "remove");
        });
        changeClass(this, "toggle");
      }
    ...
    function changeClass(element, action) {
      element.classList[action]("active-style");
      element.nextElementSibling.classList[action]("active-content");
    } // changeClass(this, "toggle");

  }); //const block = document.querySelectorAll(itemSelector);
  //
  //   block.forEach((block) => {
...
var calc = function calc(size, material, options, promocode, result) {
  ...
  function getCalc(event, elem) {
    elem.addEventListener(event, function (e) {
      var target = e.target,
          select = target.id;

      function calcFunc(res) {
        for (var key in res[select]) {
          if (elem.value === key) {
            switch (select) {
              case "size":
                sizeValue = res[select][key];
                console.log(sizeValue);
                break;

              case "material":
                materialValue = res[select][key];
                console.log(materialValue);
                break;

              case "options":
                optionsValue = res[select][key];
  ...
  getCalc("change", sizeBlock);
  getCalc("change", materialBlock);
  getCalc("change", optionsBlock);
  getCalc("input", promocodeBlock);
};
...
var filter = function filter() {
  var menu = document.querySelector(".portfolio-menu"),
      ...
      btnGgranddad = menu.querySelector(".granddad"),
  ...
  var typeFilter = function typeFilter(markTupe) {
    ...
    if (markTupe) {
      markTupe.forEach(function (item) {
  ...
  function assignEvent(btn, mark) {
    btn.addEventListener("click", function () {
      return typeFilter(mark);
    });
  }

  assignEvent(btnAll, markAll);
  assignEvent(btnLovers, markLovers);
  assignEvent(btnChef, markChef);
  assignEvent(btnGirl, markGirl);
  assignEvent(btnGuy, markGuy);
  assignEvent(btnGrandmother);
  assignEvent(btnGgranddad);
...
var showMoreStyles = function showMoreStyles(trigger, wrapper) {
  ...
  btn.addEventListener("click", function () {
    Object(_services_requests__WEBPACK_IMPORTED_MODULE_6__["getResource"])("http://localhost:3000/styles").then(function (res) {
      ...
      return console.log(error);
    }).finally(this.remove()); // setTimeout(() => {
    //   this.remove();
    // }, 200);
    // this.remove();
  });
